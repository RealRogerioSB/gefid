...
def load_extrato(n_load: int, _mci: int, value: int | str) -> pd.DataFrame:
    return engine.query(
        sql=stmt_load_mci if n_load == 0 else stmt_load_cpf_cgc if n_load == 1 else stmt_load_nome,
        show_spinner=False,
        ttl=60,
...
if st.button("**Pesquisar**", icon=":material/search:", type="primary"):
    ...
    investidor: pd.DataFrame = load_empresa(
        "nom" if nome_investidor else "cod" if mci_investidor else "cod_cpf_cgc" if cpf_cnpj_investidor else None,
        nome_investidor if nome_investidor else mci_investidor if mci_investidor else cpf_cnpj_investidor
        if cpf_cnpj_investidor else None,
    )

    extrato: pd.DataFrame = load_extrato(
        0 if mci_investidor else 1 if cpf_cnpj_investidor else 2,
        mci,
        mci_investidor if mci_investidor else cpf_cnpj_investidor if cpf_cnpj_investidor else nome_investidor,
    )
    ...
    _cnpj: str = empresa["cnpj_empresa"][0]
    _mci_investidor: int = investidor["mci_empresa"][0]
    _investidor: str = investidor["nm_empresa"][0]
    _cpf_cnpj: str = investidor["cnpj_empresa"][0]
    ...
    if len(extrato) > 0:
        for _extrato in extratos:
            consulta = extrato[extrato["pk"].eq(_extrato)].copy()
            ...
            tipo = engine.query(
                sql="SELECT t1.SG_TIP_TIT FROM DB2AEB.TIP_TIT t1 WHERE t1.CD_TIP_TIT = :tipo",
                show_spinner="Obtendo os dados, aguarde...",
                ttl=60,
    ...
    st.write(_cnpj)
    st.write(_mci_investidor)
    st.write(_investidor)
    st.write(_cpf_cnpj)
    st.write(saldo_anteriores)
    st.write(custodiantes)
    st.write(tipos)

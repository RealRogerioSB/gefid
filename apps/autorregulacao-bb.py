...
if st.session_state.btn_ac:
    ...
    invpfbra = dados[dados.TP.eq("PF") & dados.PAIS.eq("BRA")].count()[0]
    invpjbra = dados[dados.TP.eq("PJ") & dados.PAIS.eq("BRA")].count()[0]
    invest = dados[dados.PAIS.ne("BRA")].count()[0]
    acpfbra = sum(dados.TOTAL[dados.TP.eq("PF") & dados.PAIS.eq("BRA")])
    acpjbra = sum(dados.TOTAL[dados.TP.eq("PJ") & dados.PAIS.eq("BRA")])
    ...
    invmaisque1000 = dados[dados.TOTAL.gt(1000)].count()[0]
    ...
    acmaisque1000 = sum(dados.TOTAL[dados.TOTAL.gt(1000)])
    ...
    def arquivo_base_acionaria() -> None:
        ...
        worksheet_qr.write("C5", invpfbra, qtd_format)
        worksheet_qr.write("C6", invpjbra, qtd_format)
        worksheet_qr.write("C7", invest, qtd_format)
        worksheet_qr.write("C8", invpfbra + invpjbra + invest, menu_format)

        worksheet_qr.write("D4", "Ações", menu_format)
        worksheet_qr.write("D5", acpfbra, qtd_format)
        worksheet_qr.write("D6", acpjbra, qtd_format)
        worksheet_qr.write("D7", acest, qtd_format)
        worksheet_qr.write("D8", acpfbra + acpjbra + acest, menu_format)
        ...
        worksheet_qr.write("C15", invmaisque1000, qtd_format)
        worksheet_qr.write("C16", inv1a11 + inv12a50 + inv51a100 + inv101a1000 + invmaisque1000, menu_format)
        ...
        worksheet_qr.write("D15", acmaisque1000, qtd_format)
        worksheet_qr.write("D16", ac1a11 + ac12a50 + ac51a100 + ac101a1000 + acmaisque1000, menu_format)
...
if st.session_state.btn_au:
    ...
    def arquivo_siri() -> None:
        ...
        def aba_base() -> None:
            ...
            tesouBB = sum(dados.TOTAL[dados.MCI.eq(903485186)])
            tesouBBASSET = sum(dados.TOTAL[dados.MCI.eq(903721460)])
            CA = CD = DE = 0
            ...
            sheet1["G24"] = CA + CD + DE
            sheet1["G25"] = outrosconselhos

            sheet1["G52"] = ac1a11
            sheet1["G53"] = ac12a50
            sheet1["G54"] = ac51a100
            sheet1["G55"] = ac101a1000
            sheet1["G56"] = acmaisque1000

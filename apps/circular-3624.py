...
with st.columns(2)[0]:
    st.text_input(label="**PARA:**", value=email[0], key="para")
    st.text_input(label="**CC:**", value=email[1], key="cc")
    ...
    col[0].slider(
        ...
        min_value=1,
        max_value=12,
        value=hoje.month - 1 if 1 <= hoje.month - 1 else 12,
        key="mes"
    )
    col[1].selectbox(
        label="**Ano:**",
        options=range(hoje.year, 1995, -1),
        index=0 if 1 <= hoje.month - 1 else 1,
        key="ano"
    )

    if st.button("**ENVIAR**", type="primary"):
        if all([st.session_state["para"], st.session_state["cc"]]):
            preparo_xlsx(st.session_state["ano"], st.session_state["mes"])
        else:

...
if st.session_state["gerar"]:
    with st.spinner("**:material/hourglass: Preparando a tabela para exportar, aguarde...**", show_time=True):
        ...
        if len(df["cod_emissor"].unique()) > 1:
            st.toast(f"###### Identificamos que os arquivos contém dois emissores diferentes "
                     f"{df['cod_emissor'].unique()}.\nTodos os arquivos precisam ser do mesmo"
        ...
        for x in range(len(lista_dir)):
            ...
            for i in range(len(globals()[nome_df])):
                ...
                # fazendo o somatório
                globals()[nome_sheet] \
                    .write_formula(0, 10, f"=SUBTOTAL(9,K3:K{len(globals()[nome_df][i].index) + 2})", vlr_format2)
                globals()[nome_sheet] \
                    .write_formula(0, 11, f"=SUBTOTAL(9,L3:L{len(globals()[nome_df][i].index) + 2})", vlr_format2)
                globals()[nome_sheet] \
                    .write_formula(0, 12, f"=SUBTOTAL(9,M3:M{len(globals()[nome_df][i].index) + 2})", vlr_format2)

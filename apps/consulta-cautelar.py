...
st.button("**Pesquisar**", key="btn_search", type="primary", icon=":material/search:")
...
if st.session_state["btn_search"]:
    if not any([st.session_state["inscricao"], st.session_state["nome_investidor"],
    ...
    elif all([st.session_state["inscricao"], st.session_state["nome_investidor"],
              st.session_state["mci_investidor"], st.session_state["cpf_cnpj_investidor"]]):
        ...
        if not acionistas.empty:
            ...
            if len(certificados) > 0:
                ...
                st.toast("###### Não foram encontrados dados para a pesquisa realizada", icon=":material/warning:")

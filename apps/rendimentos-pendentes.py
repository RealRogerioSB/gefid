...
def load_active(active: str) -> dict[int, str]:
    df = engine.query(
        sql=f"""SELECT t1.CD_CLI_EMT AS MCI, STRIP(t2.NOM) AS NOM
...
def load_data(_mci: int) -> pd.DataFrame:
    return engine.query(
        sql="""SELECT t1.CD_CLI_EMT AS MCI, STRIP(t1.SG_EMP) AS SIGLA, STRIP(t2.NOM) AS EMPRESA, t2.COD_CPF_CGC AS CNPJ
               FROM DB2AEB.PRM_EMP t1 INNER JOIN DB2MCI.CLIENTE t2 ON t2.COD = t1.CD_CLI_EMT
...
if st.session_state["btn_view"]:
    ...
    if not get_view.empty:
        ...
        st.dataframe(get_view, hide_index=True)
    else:
...
if st.session_state["btn_csv"]:
    ...
    if not get_csv.empty:
        ...
        get_csv.to_csv(path_or_buf=f"static/escriturais/@deletar/{sigla}-Rendimentos Pendentes.csv", index=False)
...
if st.session_state["btn_excel"]:
    ...
    if not get_xlsx.empty:
        ...
        if len(get_xlsx) <= int(1e6):
            get_xlsx.to_excel(
                excel_writer=f"static/escriturais/@deletar/{sigla}-Rendimentos Pendentes.xlsx",
                index=False,
                engine="xlsxwriter"
            )
            ...
            get_xlsx[:int(1e6)].to_excel(
                excel_writer=f"static/escriturais/@deletar/{sigla}-Rendimentos Pendentes-parte1.xlsx",
                index=False,
                engine="xlsxwriter"
            )

            get_xlsx[int(1e6):int(2e6)].to_excel(
                excel_writer=f"static/escriturais/@deletar/{sigla}-Rendimentos Pendentes-parte2.xlsx",
                index=False,
                engine="xlsxwriter"
            )

            get_xlsx[int(2e6):].to_excel(
                excel_writer=f"static/escriturais/@deletar/{sigla}-Rendimentos Pendentes-parte3.xlsx",
                index=False,
                engine="xlsxwriter"
            )
